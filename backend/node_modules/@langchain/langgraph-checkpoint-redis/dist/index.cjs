	...
	async *list(config, options) {
		...
		if (options?.filter !== void 0) {
			...
			if (config?.configurable?.thread_id) {
				const threadId = require_utils.escapeRediSearchTagValue(config.configurable.thread_id);
			...
			if (config?.configurable?.checkpoint_ns !== void 0) {
				...
				if (checkpointNs === "") queryParts.push(`(@checkpoint_ns:{__empty__})`);
					...
					const escapedNs = require_utils.escapeRediSearchTagValue(checkpointNs);
